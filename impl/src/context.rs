...
impl<'a> ContextOpts<'a> {
    fn inherit_from(mut self, attrs: &'a Attrs) -> Self {
        macro_rules! update_opt {
            () => {};
            (+ $opt:ident, $($rest:tt)*) => {
                self.$opt.extend(attrs.thisctx.$opt.iter());
                update_opt!($($rest)*);
            };
            (= $opt:ident, $($rest:tt)*) => {
                if self.$opt.is_none() {
                    self.$opt = attrs.thisctx.$opt.as_ref();
                }
                update_opt!($($rest)*);
            };
            (=* $opt:ident, $($rest:tt)*) => {
                if self.$opt.is_none() {
                    self.$opt = attrs.thisctx.$opt;
                }
                update_opt!($($rest)*);
            };
        }
    ...
    fn new(
        ...
        for field in fields {
            ...
            if source_field.is_none() && (is_transparent || field.is_source()) {
                ...
                ty = if intersection.is_empty() && field.attrs.thisctx.generic.unwrap_or(generic) {
        ...
        Some(Self {
            ...
            name: {
                ...
                match (opts.suffix, container_variant.is_some()) {
                    ...
                    (Some(Or(s)), _) => format_ident!("{}{}", name, s),
                }
            },
